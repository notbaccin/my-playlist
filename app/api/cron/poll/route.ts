...
export async function GET() {
  ...
  try {
    ...
    if (!masterRes.ok) {
      throw new Error(`Spotify API respondeu com status: ${masterRes.status}`);
    }
    ...
    if (!spotifyRes.ok) {
      throw new Error(`Spotify API falhou no lote de faixas: ${spotifyRes.status}`);
    }
    ...
    console.warn("⚠️ Spotify indisponível ou Rate Limited. Ativando Fallback de Segurança do Supabase:", syncError.message);
    usedCache = true;
  }
  ...
  if (tracksToReturn.length === 0) {
    try {
      ...
      if (!dbError && dbTracks && dbTracks.length > 0) {
        tracksToReturn = dbTracks;
      }
    } catch (dbError) {
      console.error("Erro crítico ao tentar acessar dados de fallback do Supabase:", dbError);
    }
  }
